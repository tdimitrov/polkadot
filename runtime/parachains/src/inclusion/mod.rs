...
impl<T: Config> Pallet<T> {
				...
				candidates.iter().enumerate()
			{
				...
				for (i, assignment) in scheduled[skip..].iter().enumerate() {
					...
					if para_id == assignment.para_id {
						...
						{
							let maybe_amount_validated = primitives::active_api::check_candidate_backing(
								&backed_candidate,
								&signing_context,
								group_vals.len(),
								|intra_group_vi| {
	...
	pub(crate) fn check_validation_outputs_for_runtime_api(
		para_id: ParaId,
		validation_outputs: primitives::active_api::CandidateCommitments,
	) -> bool {
...
impl<T: Config> CandidateCheckContext<T> {
	...
	fn check_validation_outputs(
		...
		para_id: ParaId,
		head_data: &HeadData,
		new_validation_code: &Option<primitives::active_api::ValidationCode>,
		processed_downward_messages: u32,
		upward_messages: &[primitives::active_api::UpwardMessage],
		hrmp_watermark: T::BlockNumber,
		horizontal_messages: &[primitives::active_api::OutboundHrmpMessage<ParaId>],
