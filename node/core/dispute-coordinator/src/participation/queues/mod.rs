...
impl Ord for CandidateComparator {
	fn cmp(&self, other: &Self) -> Ordering {
		// handle corner cases when one (or both) of the relay parent block numbers are `None`
		if self.relay_parent_block_number.is_none() && other.relay_parent_block_number.is_none() {
			// if both relay parents are unknown - just compare the candidate hashes
			return self.candidate_hash.cmp(&other.candidate_hash)
		}
